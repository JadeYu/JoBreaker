...
import pandas as pd
import joblib
from functions import status_to_words, raw_cleaning, contributing_words

text_clf = joblib.load("models/text_clf_medium.pkl")
#keywords = pd.read_csv("keywords_min.csv", index_col=0)
keywords = pd.read_csv("keywords_medium_30.csv", index_col=0)

@app.route('/')
@app.route('/index')
def index():
    return render_template("index.html",
       title = 'Home', user = { 'nickname': 'Jade' },
       )
...
def model_result():
  ...
  contri_words = contributing_words(cleaned_words, keywords)
